...
Auth lifecycle/webhook side-effects are centralized in `src/lib/auth/side-effects.ts`, which delegates subscription sync and checkout product lookup to the billing feature.
...
4. **Deletion** → auth `onUserDeleted(userId)` attempts Polar customer cleanup after local user deletion. Missing customers are idempotent in the gateway; other cleanup failures propagate through the delete-user hook.
