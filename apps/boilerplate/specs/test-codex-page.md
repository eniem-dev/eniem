...
Add a static `/test-codex` page under the `(marketing)` route group. This is a smoke test for the codex adapter's plan+build pipeline.

## Requirements

### Must Have

- Create `src/app/(marketing)/test-codex/page.tsx`
- Page exports metadata using `createMetadata` + `getDefaultMetadata` from `@/lib/metadata`
- Page renders a `<div>` with heading "Test Codex Page" and a paragraph "This page was built by the codex adapter."
...
- [ ] `pnpm build` passes
- [ ] `pnpm lint` passes
- [ ] Page file exists at `src/app/(marketing)/test-codex/page.tsx`
- [ ] Page renders heading and paragraph text

## Beads Rules

- Epic title: `test-codex-page: Add /test-codex placeholder page`
- Epic must use `--type=epic`
- Create exactly **1 task** under the epic
- Task title: `Create /test-codex page component`
