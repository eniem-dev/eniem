...
Add a static `/test-gemini` page under the `(marketing)` route group. This is a smoke test for the gemini adapter's plan+build pipeline.

## Requirements

### Must Have

- Create `src/app/(marketing)/test-gemini/page.tsx`
- Page exports metadata using `createMetadata` + `getDefaultMetadata` from `@/lib/metadata`
- Page renders a `<div>` with heading "Test Gemini Page" and a paragraph "This page was built by the gemini adapter."
...
- [ ] `pnpm build` passes
- [ ] `pnpm lint` passes
- [ ] Page file exists at `src/app/(marketing)/test-gemini/page.tsx`
- [ ] Page renders heading and paragraph text

## Beads Rules

- Epic title: `test-gemini-page: Add /test-gemini placeholder page`
- Epic must use `--type=epic`
- Create exactly **1 task** under the epic
- Task title: `Create /test-gemini page component`
