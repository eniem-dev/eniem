...
By default, `eni ai init` clones boilerplate files via HTTPS. If you prefer SSH, use the `--ssh` flag:

```bash
# Default: HTTPS (works everywhere, no setup needed)
eni ai init
...
The same flag also works with the main `eni [project-name]` scaffolding command.
