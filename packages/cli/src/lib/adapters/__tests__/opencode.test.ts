...
describe("opencode adapter", () => {
  ...
  it("spawns opencode with correct arguments", async () => {
    const mock = createMockSubprocess();
    vi.mocked(execa).mockReturnValue(mock as never);

    const runner = opencodeAdapter.run("build the thing", {
      args: ["--extra"],
      cwd: "/test",
    });
  ...
  it("parses text events and calls onText callback", async () => {
    ...
    const onText = vi.fn();
    const runner = opencodeAdapter.run("test prompt", { onText });
  ...
  it("parses tool_use events and calls onToolUse callback", async () => {
    ...
    const runner = opencodeAdapter.run("test prompt", { onToolUse });
    await runner.result;
  ...
  it("detects sentinel in output", async () => {
    const textEvent = JSON.stringify({
      type: "text",
      part: { text: "Done :::ENI_DONE:::" },
    });

    const mock = createMockSubprocess({ stdoutLines: [textEvent] });
    vi.mocked(execa).mockReturnValue(mock as never);

    const runner = opencodeAdapter.run("test prompt");
    const result = await runner.result;
  ...
  it("throws descriptive error when binary not found (ENOENT)", async () => {
    const error = Object.assign(new Error("spawn opencode ENOENT"), {
      code: "ENOENT",
    });
    const mock = createMockSubprocess({ error });
    vi.mocked(execa).mockReturnValue(mock as never);
    ...
    await expect(runner.result).rejects.toThrow("OpenCode CLI not found");
  });
